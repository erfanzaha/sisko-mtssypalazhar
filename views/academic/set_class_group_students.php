...
function SetAsAlumni() {
	...
	if (student_ids.length) {
		eModal.confirm('Apakah anda yakin ' + student_ids.length + ' data Peserta Didik yang terceklis akan diatur sebagai alumni tahun ' + end_date +'?', 'Konfirmasi').then(function() {
			var values = {
...
function getFromSourceList() {
	...
	$.post(_BASE_URL + 'academic/class_group_students/get_students', values, function( response ) {
		...
		if (rows.length) {
			...
			str += '<th>NOMOR INDUK Peserta Didik</th>';
...
function saveToDestinationClass() {
	...
	if (from_class_group_id == to_class_group_id && from_academic_year_id == to_academic_year_id) {
		_H.Notify('warning', 'Kelas dan Tahun Pelajaran tujuan tidak boleh sama dengan Kelas dan Tahun Pelajaran asal.');
	} else {
		...
		if (student_ids.length) {
			eModal.confirm('Apakah anda yakin ' + student_ids.length + ' data Peserta Didik yang terceklis akan diatur sebagai Peserta Didik Kelas ' + class_name + ' Tahun Pelajaran ' + academic_year + '?', 'Konfirmasi').then(function() {
				$('.origin-class-overlay').show();
				var values = {
					student_ids: student_ids.join(','),
				...
				$.post(_BASE_URL + 'academic/class_group_students/set_class_group_students', values, function( response ) {
					var res = _H.StrToObject( response );
					_H.Notify(res.status, res.message);
					getFromSourceList();
					getFromTargetList();
...
function DeleteFromDestinationClass() {
	...
	if (student_ids.length) {
		eModal.confirm('Apakah anda yakin ' + student_ids.length + ' data Peserta Didik yang terceklis akan dihapus?', 'Konfirmasi').then(function() {
...
<section class="content">
	<div class="callout">
		...
		<ul>
			...
			<li>Jika yang dipilih <strong>"NAMA KELASNYA"</strong>, maka akan diminta untuk memilih <strong>"TAHUN PELAJARAN"</strong></li>
			<li>Setelah data peserta didik tampil, silahkan <strong>"CEKLIS"</strong> peserta didik yang akan diatur kelasnya</li>
			<li>Setelah selesai memilih peserta didik yang akan dipindah kelasnya, silahkan pilih <strong>"KELAS TUJUAN"</strong> dan <strong>"TAHUN PELAJARAN"</strong> pada tabel sebelah kanan.</li>
			<li>Terakhir klik tombol <strong>"PINDAH KE KELAS TUJUAN"</strong></li>
			<li>Jika terjadi kelasahan penginputan data, silahkan pilih <strong>"KELAS TUJUAN"</strong> dan <strong>"TAHUN PELAJARAN"</strong> pada tabel sebelah kanan, kemudian ceklis peserta didik yang akan dihapus dari kelas tersebut dan terakhir klik tombol <strong>"HAPUS"</strong></li>
		</ul>
	</div>
	<div class="row">
				...
				<div class="box-body">
					<form class="form-horizontal">
						...
						<div class="form-group from_academic_year_id" style="display: none;">
							<label for="from_academic_year_id" class="col-sm-4 control-label">Tahun Pelajaran</label>
							<div class="col-sm-8">
