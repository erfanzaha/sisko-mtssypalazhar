...
DS.SchoolLevels = _H.StrToObject('<?=get_options('school_level')?>');
DS.OwnershipStatus = _H.StrToObject('<?=get_options('institution_lifter')?>');
var _grid = 'OPTIONS',
...
_form23 = _grid + '_FORM23'; // Foto Kepala Sekolah
   ...
   fields: [
      {
         ...
         renderer: function( row ) {
            ...
            if (row.setting_variable == 'headmaster_photo') {
               return UPLOAD(_form23 + '.OnUpload(' + row.id + ')', 'image', 'Upload Foto Kepala Sekolah');
            }
         },
         exclude_excel: true,
         sorting: false
...
new FormBuilder( _form2 , {
   controller:'settings/school_profile',
   fields: [
      { label:'Nama Sekolah', name:'setting_value' }
   ]
});
...
new FormBuilder( _form23, {
   controller:'settings/school_profile',
   fields: [
      { label:'Foto Kepala Sekolah', name:'setting_value' }
   ]
});
