		...
		if ( page <= parseInt(total_page) ) {
			...
			$.post( _BASE_URL + 'public/gallery_photos/get_albums', data, function( response ) {
				...
				for (var z in rows) {
					...
					str += '<button type="button" onclick="photo_preview(' + row.id + ')" class="btn btn-sm btn-success rounded-0"><i class="fa fa-search"></i></button>';
					str += '</div>';
					str += '</div>';
					str += '</div>';
					str += '</div>';
...
<div class="col-lg-12 col-md-12 col-sm-12">
			...
			<div class="col-md-4 mb-4 albums">
				<div class="card h-100 shadow border border-secondary rounded-0">
					...
					<div class="card-footer">
						<button type="button" onclick="photo_preview(<?=$row->id?>)" class="btn btn-sm btn-success rounded-0 float-right"><i class="fa fa-search"></i></button>
					</div>
				</div>
			</div>
		<?php } ?>
	...
	<div class="justify-content-between align-items-center float-right mb-3 w-100 more-albums">
		<button type="button" onclick="get_albums(); return false;" class="btn btn-success rounded-0 float-right"><i class="fa fa-refresh"></i> Tampilkan Lebih Banyak</button>
	</div>
</div>
