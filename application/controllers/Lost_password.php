		...
		if ($this->input->is_ajax_request()) {
			if ($this->validation()) {
				...
				if (NULL == $query) {
					...
					$message .= "Admin";
					$message .= "<br>";
					$message .= __session('school_profile')['school_name'];
					$config = array(
						'protocol' => 'smtp',
						'smtp_host' => __session('mail_server')['smtp_host'], 
						'smtp_port' => __session('mail_server')['smtp_port'],
						'smtp_user' => __session('mail_server')['smtp_user'],
						'smtp_pass' => __session('mail_server')['smtp_pass'],
						'smtp_crypto' => 'ssl', 
						'mailtype' => 'html',
						'smtp_timeout' => '4',
						'charset' => 'iso-8859-1',
					...
					$this->email->from(__session('school_profile')['email'], __session('school_profile')['school_name']);
					$this->email->to($query['email']);
					$this->email->subject('Lupa Kata Sandi');
					$this->email->message($message);
					if ($this->email->send()) {
						$update = $this->m_users->set_forgot_password_key($query['email'], $forgot_password_key);
						if ($update) {
						...
						$this->vars['status'] = 'warning';
						$this->vars['message'] = 'Tautan untuk mengubah kata sandi tidak terkirim. Silahkan kirim email ke ' . __session('school_profile')['email'];
					}
				}
			} else {
