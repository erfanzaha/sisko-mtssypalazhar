...
class Academic_years extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'Tahun Pelajaran';
		...
		if ($this->input->is_ajax_request()) {
			...
			if ($this->validation()) {
				...
				if (_isNaturalNumber( $id )) {
					...
					if ($this->vars['status'] == 'success' && filter_var($dataset['current_semester'], FILTER_VALIDATE_BOOLEAN)) {
						$this->m_academic_years->deactivate($id, 'current_semester');
					}
					if ($this->vars['status'] == 'success' && filter_var($dataset['admission_semester'], FILTER_VALIDATE_BOOLEAN)) {
						$this->m_academic_years->deactivate($id, 'admission_semester');
					}
				} else {
					if (filter_var($dataset['current_semester'], FILTER_VALIDATE_BOOLEAN)) {
						$this->m_academic_years->deactivate(0, 'current_semester');
					}
					if (filter_var($dataset['admission_semester'], FILTER_VALIDATE_BOOLEAN)) {
						$this->m_academic_years->deactivate(0, 'admission_semester');
					}
	...
	private function validation() {
		...
		$val->set_rules('academic_year', 'Tahun Pelajaran', 'trim|required|min_length[9]|max_length[9]|callback_format_check');
		...
		$val->set_rules('admission_semester', 'Semester PPDB', 'trim|required|in_list[true,false]');
