...
class Banners extends Admin_Controller {
		...
		if ($this->input->is_ajax_request()) {
			...
			if (_isNaturalNumber( $id )) {
				...
				if (!$this->upload->do_upload('file')) {
					...
					$update = $this->model->update($id, $this->table, ['link_image' => $upload['file_name']]);
					if ($update) {
						// chmood new file
						@chmod(FCPATH.'media_library/banners/'.$upload['file_name'], 0777);
						// chmood old file
						@chmod(FCPATH.'media_library/banners/'.$file_name, 0777);
						// unlink old file
						@unlink(FCPATH.'media_library/banners/'.$file_name);
						// resize new image
						$this->image_resize(FCPATH.'media_library/banners', $upload['file_name']);
					}
	...
	private function image_resize($path, $file_name) {
		...
		$config['width'] = (int) __session('media')['banner_width'];
		// $config['height'] = (int) __session('media')['banner_height'];
