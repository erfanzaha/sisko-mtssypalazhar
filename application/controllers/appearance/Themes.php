    	...
    	if ($this->input->is_ajax_request()) {
	    	...
	    	if (_isNaturalNumber( $id )) {
				...
				if ( ! $this->upload->do_upload('file')) {
					...
					if ($query->theme_folder == $upload['raw_name']) {
						$zip = new ZipArchive;
						if ($zip->open(VIEWPATH . 'themes/' . $upload['file_name'])) {
							$zip->extractTo(VIEWPATH . 'themes/');
							$zip->close();
							// chmod Theme Folder
							$this->chmod_themes('./views/themes/'.$upload['raw_name']);
					...
					// Delete ZIP File
					@chmod('./views/themes/' . $upload['file_name'], 0777);
					@unlink('./views/themes/' . $upload['file_name']);
				}
	    	} else {
				$this->vars['status'] = 'error';
