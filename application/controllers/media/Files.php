...
class Files extends Admin_Controller {
	...
	public function index() {
		...
		$this->vars['file_category_dropdown'] = json_encode($this->m_file_categories->dropdown(), self::REQUIRED_FLAGS);
    ...
    public function upload() {
    	if ($this->input->is_ajax_request()) {
	    	...
	    	if (_isNaturalNumber( $id )) {
				...
				$file_allowed_types = explode(',', __session('media')['file_allowed_types']);
				$allowed_types = [];
				...
				$config['max_size'] = (int) __session('media')['upload_max_filesize'];
				...
				if ( ! $this->upload->do_upload('file')) {
					...
					$this->vars['full_path'] = $upload['full_path'];
				}
			}
	    	$this->output
				->set_content_type('application/json', 'utf-8')
