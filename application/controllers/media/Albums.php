...
class Albums extends Admin_Controller {
	...
	*/
	private function album_cover_resize($path, $file_name) {
		...
		$config['width'] = (int) __session('media')['album_cover_width'];
		$config['height'] = (int) __session('media')['album_cover_height'];
	...
	*/
	...
	private function image_resize($path, $file_name, $size = 'large') {
		$settings = [
			'thumbnail_size_height' => __session('media')['thumbnail_size_height'],
			'thumbnail_size_width' => __session('media')['thumbnail_size_width'],
			'medium_size_height' => __session('media')['medium_size_height'],
			'medium_size_width' => __session('media')['medium_size_width'],
			'large_size_height' => __session('media')['large_size_height'],
			'large_size_width' => __session('media')['large_size_width']
		];
		$this->load->library('image_lib');
		$config['image_library'] = 'gd2';
		$config['source_image'] = $path .'/'.$file_name;
