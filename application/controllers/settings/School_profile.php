...
class School_profile extends Admin_Controller {
		...
		if ($this->input->is_ajax_request()) {
			...
			if (_isNaturalNumber( $id )) {
				...
				if (!$this->upload->do_upload('file')) {
					...
					$update = $this->model->update($id, $this->table, ['setting_value' => $upload['file_name']]);
					if ($update) {
						// chmood new file
						@chmod(FCPATH.'media_library/images/'.$upload['file_name'], 0777);
						// chmood old file
						...
						if ($query->setting_variable != 'headmaster_photo') {
							$this->image_resize('./media_library/images', $upload['file_name'], $query->setting_variable);
						}
					}
	 ...
	 * @param String $siz$setting_variablee
	 * @return Void
	 */
	 private function image_resize($path, $file_name, $setting_variable = 'headmaster_photo') {
	 	$settings = [
	 		'user_photo_height' => NULL !== __session('media')['user_photo_height'] ? __session('media')['user_photo_height'] : 344,
	 		'user_photo_width' => NULL !== __session('media')['user_photo_width'] ? __session('media')['user_photo_width'] : 252,
	 		'logo_height' => NULL !== __session('media')['logo_height'] ? __session('media')['logo_height'] : 120,
	 		'logo_width' => NULL !== __session('media')['logo_width'] ? __session('media')['logo_width'] : 120
	 	];
