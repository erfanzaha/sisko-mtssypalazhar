...
class M_posts extends CI_Model {
	...
	*/
	...
	public function get_related_posts($post_categories = '', $id = 0) {
		...
		$this->db->group_end();
		$this->db->order_by('x1.created_at', 'DESC');
		$this->db->limit((__session('reading')['post_related_count'] > 0 ? __session('reading')['post_related_count'] : 5));
		return $this->db->get(self::$table . ' x1');
	}
	...
	*/
	...
	public function feed() {
		...
		$this->db->limit((__session('reading')['post_rss_count'] > 0 ? __session('reading')['post_rss_count'] : 10));
		return $this->db->get(self::$table);
	}
