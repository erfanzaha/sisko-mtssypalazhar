...
class M_admission_selection_process extends CI_Model {
	...
	public function get_prospective_students($admission_year_id = 0, $admission_type_id = 0, $major_id = 0, $return_type = 'count', $limit = 0, $offset = 0) {
		...
		$min_birth_date = __session('admission')['min_birth_date'];
		$max_birth_date = __session('admission')['max_birth_date'];
		...
		if ( $return_type === 'count' ) return $this->db->count_all_results('users x1');
	 ...
	 * @return Boolean
	 */
	public function selection_process($admission_year_id = 0, $admission_type_id = 0, $selection_result = '', $student_ids = []) {
		// Define Admission Year
		$admission_year = $this->model->get_admission_year($admission_year_id);
		// Default Quota
		$admission_quota = 0;
		...
		if ($selection_result != 'unapproved') {
			...
			if (($admission_quota - $approved) < count($student_ids)) {
				return [
					'status' => 'warning',
					'message' => 'Kuota Penerimaan tidak mencukupi. Silahkan periksa kembali pengaturan Kuota Penerimaan.'
				];
			}
		}
