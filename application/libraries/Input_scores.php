...
class Input_scores extends TCPDF {
	...
	public function pdf(array $params = []) {
		...
		foreach($params['students'] as $row) {
			...
			$students .= '<td width="10%" align="center">' . presence($row['assignment_score']) . '</td>';
			$students .= '<td width="10%" align="center">' . presence($row['mid_score']) . '</td>';
			$students .= '<td width="10%" align="center">' . presence($row['exam_score']) . '</td>';
